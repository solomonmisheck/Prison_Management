    ...
    <div class="container-fluid px-4">
        <div class="card mt-4">
            <div class="card-header">
                <h4 class="">Edit Health Record</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ url('admin/edit-health_record/'.$health_record->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-sm" aria-label=".form-select-lg example" required="required" name="inmate_id">
                            ...
                            <option {{ old('inmate_id', $health_record->inmate_id) ==  $inmate->id ? 'selected' : ''}} value="{{ $inmate->id }}">{{ $inmate->firstname .' '.$inmate->lastname }}</option>
                            @endforeach                            
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-sm" aria-label=".form-select-lg example" required="required" name="disease_id">
                            ...
                            <option {{ old('disease_id', $health_record->disease_id) ==  $disease->id ? 'selected' : ''}} value="{{ $disease->id }}">{{ $disease->name }}</option>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" id="date_from" name="date_from" value="{{ $health_record->date_from }}" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label for="">End Date</label>
                        <input type="date" id="date_to" name="date_to" value="{{ $health_record->date_to }}" class="form-control">
                    </div>
                    <div class="mb-3">
                        <label for="">Treatment Done</label>
                        <input type="text" name="treatment_done" value="{{ $health_record->treatment_done }}" class="form-control">
                    </div>
                    <div class="mb-3">
